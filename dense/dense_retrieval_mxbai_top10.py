...
OUTPUT_PATH = "../embeddings + results/dense_mxbai_embed_large_results_top10.md"
...
assert len(df) == len(cv_embeddings), \
    "Mismatch between dataset rows and embeddings!"
...
with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
    ...
    log("# Dense Retrieval Results - MXBAI Embed Large", f)
    log(f"\nModel: {MODEL_NAME}", f)
    log(f"Top K: {TOP_K}", f)
    ...
    for category, queries in query_categories.items():
        ...
        for query in queries:
            ...
            for rank, (idx, score) in enumerate(
                ...
                log(
                    f"- Candidate ID: {safe_value(row.get('id', row.get('ID', idx)))}",
                    f
                )
