...
EMBEDDINGS_PATH = "../data/cv_embeddings_bge_m3_final.npy"
OUTPUT_PATH = "../embeddings + results/dense_bge_m3_results_top10.md"
...
cv_embeddings = cv_embeddings / (
    np.linalg.norm(cv_embeddings, axis=1, keepdims=True) + 1e-12
)
...
with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
    log("# Dense Retrieval Results - BGE-M3", f)
    log(f"\nModel: {MODEL_NAME}", f)
    log(f"Top K: {TOP_K}", f)
