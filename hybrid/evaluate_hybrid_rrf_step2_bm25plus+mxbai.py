...
HYBRID_PATH = "../data/hybrid_rrf_rankings_bm25plus_mxbai.csv"
QRELS_PATH = "../data/qrels_annotations_top10_final.xlsx"

OUTPUT_PER_QUERY = "../data/metrics_hybrid_rrf_bm25plus_mxbai_per_query.csv"
OUTPUT_SUMMARY = "../data/metrics_hybrid_rrf_bm25plus_mxbai_summary.csv"
...
def ndcg_at_k(binary_rels, k):
    ...
    dcg = np.sum(
        np.array(rels, dtype=float) / np.log2(np.arange(2, len(rels) + 2))
    )

    idcg = np.sum(
        np.array(ideal, dtype=float) / np.log2(np.arange(2, len(ideal) + 2))
    )
...
for query_id, group in eval_df.groupby("query_id"):
    ...
    row = {
        ...
        "Precision@10":
            precision_at_k(binary_rels, 10),

        "Recall@10":
            recall_at_k(binary_rels, total_relevant, 10),

        "MAP@10":
            average_precision_at_k(binary_rels, 10),

        "MRR@10":
            reciprocal_rank_at_k(binary_rels, 10),

        "NDCG@10":
            ndcg_at_k(binary_rels, 10),
    }
