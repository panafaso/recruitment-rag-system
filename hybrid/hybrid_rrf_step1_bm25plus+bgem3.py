...
df = df[df["system"].isin(["BM25Plus", "BGE-M3"])]
...
for query_id, group in df.groupby("query_id"):
    ...
    for new_rank, (doc_id, rrf_score) in enumerate(ranked_docs[:10], start=1):
        hybrid_rows.append({
            "query_id": query_id,
            "system": "Hybrid_RRF_BM25Plus_BGE-M3",
            "rank": new_rank,
            "score": rrf_score,
            "candidate_id": doc_id
        })
...
hybrid_df.to_csv("../data/hybrid_rrf_rankings_bm25plus_bgem3.csv", index=False)

print("Hybrid RRF rankings saved to ../data/hybrid_rrf_rankings_bm25plus_bgem3.csv")
